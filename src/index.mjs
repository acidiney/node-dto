...
import schema from './utils/props_schema.mjs';
import AvailableTypes from './utils/types.mjs';
...
function validateLine(entry, data, index = null) {
  if (entry.required && data === null && data === undefined)
    throw new ValidateException(
      `Field ${entry.name} is required${typeof index === 'number' ? ` - on index #${index}` : ''
      }!`
    );
  ...
  if (data && !result)
    throw new ValidateException(
      `Field ${entry.name} with value ${data}, is not typeof ${entry.type}${typeof index === 'number' ? ` - on index #${index}` : ''
      }!`
    );
...
function _validate(input, dto, index = null) {
  ...
  requiredProps.forEach((prop) => {
    if (!keys.includes(prop)) {
      throw new ValidateException(
        `Field ${prop} is required${typeof index === 'number' ? ` - on index #${index}` : ''
        }!`
      );
    }
  });
...
export function MakeDto(params) {
